...
```bash
circleci config pack ./.circleci/pull-request-src > ./circleci/pull-request.yml
```

This command:
1. Takes the source configuration from `./.circleci/pull-request-src`
2. Packs it into a single YAML file
3. Outputs the result to `./circleci/pull-request.yml`

## File Structure

- `pull-request-src/` - Source configuration files (modify these)
- `pull-request.yml` - Generated configuration file (auto-generated, do not edit manually)

## Development Workflow

1. Make changes to files in `pull-request-src/`
2. The lint-staged hook will automatically regenerate `pull-request.yml`
3. Commit both the source changes and the generated file

**Note:** Always commit both the source files and the generated `pull-request.yml` file together to ensure the CircleCI configuration stays in sync. 